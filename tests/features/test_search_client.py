...
class TestSearchClient(unittest.TestCase):
    ...
    def test_api_keys(self):
        response = self.client.add_api_key(
            ["search"],
            {
        ...
        self.assertEqual(api_key["value"], response.raw_response["key"])

        api_keys = list(
            map(lambda facet: api_key["value"], self.client.list_api_keys()["keys"])
        )

        self.assertIn(api_key["value"], api_keys)

        self.client.update_api_key(api_key["value"], {"maxHitsPerQuery": 42}).wait()
    ...
    def test_get_logs(self):
        ...
        logs = self.client.get_logs({"length": 2, "offset": 0, "type": "all"})["logs"]
    ...
    def test_stopwords_dictionaries(self):
        ...
        self.assertEqual(
            client.search_dictionary_entries("stopwords", entry_id)["nbHits"], 0
        )

        entry = {"objectID": entry_id, "language": "en", "word": "down"}
        ...
        client.save_dictionary_entries("stopwords", [entry]).wait()
        self.assertEqual(
            client.search_dictionary_entries("stopwords", entry_id)["nbHits"], 1
        )
        ...
        stopwords_settings = {
            "disableStandardEntries": {"stopwords": {"en": True, "fr": True}}
        }
    ...
    def test_plurals_dictionaries(self):
        ...
        entry_id = "".join(random.choice(string.ascii_letters) for i in range(10))
        self.assertEqual(
            client.search_dictionary_entries("plurals", entry_id)["nbHits"], 0
        )

        entry = {"objectID": entry_id, "language": "fr", "words": ["cheval", "chevaux"]}
