...
class TestRequestOptions(unittest.TestCase):
    ...
    def test_with_options(self):
        self.assertIsInstance(RequestOptions.create(self.config), RequestOptions)

    def test_default_user_agent(self):
        self.assertEqual(
            RequestOptions.create(self.config).headers["User-Agent"], UserAgent.get()
        )

    def test_headers(self):
        self.assertEqual(self.request_options.headers["User-Agent"], "foo")
        self.assertEqual(self.request_options.headers["X-FORWARDED-FOR"], "1.1.1.1")

        # Default Value
        self.assertEqual(
            self.request_options.headers["Content-Type"], "application/json"
        )

        # Custom header set on the config
        self.assertEqual(self.request_options.headers["Foo-Bar"], "foo-bar")
    ...
    def test_data(self):
        self.assertEqual(self.request_options.data["bodyParam"], "bar")
