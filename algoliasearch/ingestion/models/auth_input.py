    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, AuthGoogleServiceAccount):
            error_messages.append(
                f"""Type '{type(v)}' is not 'AuthGoogleServiceAccount'"""
            )
        else:
            match += 1
        # validate data type: AuthBasic
        if not isinstance(v, AuthBasic):
            error_messages.append(f"""Type '{type(v)}' is not 'AuthBasic'""")
        else:
            match += 1
        # validate data type: AuthAPIKey
        if not isinstance(v, AuthAPIKey):
            error_messages.append(f"""Type '{type(v)}' is not 'AuthAPIKey'""")
        else:
            match += 1
        # validate data type: AuthOAuth
        if not isinstance(v, AuthOAuth):
            error_messages.append(f"""Type '{type(v)}' is not 'AuthOAuth'""")
        else:
            match += 1
        # validate data type: AuthAlgolia
        if not isinstance(v, AuthAlgolia):
            error_messages.append(f"""Type '{type(v)}' is not 'AuthAlgolia'""")
        else:
            match += 1
        if match > 1:
            # more than 1 match
