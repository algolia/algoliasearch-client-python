...
class IngestionClient:
    ...
    async def chunked_push(
        ...
        """
        Helper: Chunks the given `objects` iterable in subset of 1000 elements max in order to make it fit in `push` requests by leveraging the Transformation pipeline setup in the Push connector (https://www.algolia.com/doc/guides/sending-and-managing-data/send-and-update-your-data/connectors/push/). Accepts any iterable, including generators.
        """
        if batch_size < 1:
        ...
        async def _wait(start: int, end: int) -> None:
        ...
        it = iter(objects)
        while records := list(islice(it, batch_size)):
            responses.append(
                await self.push(
            ...
            if wait_for_tasks and len(responses) - offset >= wait_batch_size:
                await _wait(offset, offset + wait_batch_size)
                offset += wait_batch_size

        if wait_for_tasks:
            await _wait(offset, len(responses))
...
class IngestionClientSync:
    ...
    def chunked_push(
        ...
        def _wait(start: int, end: int) -> None:
            for response in responses[start:end]:
        ...
        while records := list(islice(it, batch_size)):
            ...
            if wait_for_tasks and len(responses) - offset >= wait_batch_size:
                _wait(offset, offset + wait_batch_size)
                offset += wait_batch_size

        if wait_for_tasks:
            _wait(offset, len(responses))
